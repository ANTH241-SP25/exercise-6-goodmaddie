...
ggplot(NHANESdatabase, aes(x=BMIcat)) + geom_bar(fill="pink") + labs(title="BMIcategories", x="BMIcat", y="counts")
...
BMIcat_table <- with(NHANESdatabase, table(BMIcat))
...
c. Construct bar charts for one ordinal (or nominal) variable in the code chunk below: 

```{r}

HHIncome_table <- with(NHANESdatabase, table(HHIncome))
prop.table(HHIncome_table)
prop.table(HHIncome_table)*100
HHIncome_barchart <- as.data.frame(prop.table(HHIncome_table)*100)
ggplot(HHIncome_barchart, aes(HHIncome, Freq)) + geom_col()
```
...
table(NHANESdatabase$Gender, NHANESdatabase$BMIcat)
#Stacked bar chart:
#ggplot(data, aes(x=dependent variable, fill=independent variable)) + geom_bar(position = "stack") + labs(title="", x="", y="")
ggplot(NHANESdatabase, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + labs(title = "stacked bar chart", x="BMI Categories by Gender", y="counts")
ggplot(NHANESdatabase, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + facet_wrap(~Race) + labs(title = "stacked bar chart", x="BMI Categories by Gender", y="counts")
ggplot(NHANESdatabase, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "stack") + facet_grid(~Race) + labs(title = "stacked bar chart", x="BMI Categories by Gender", y="counts")
#grouped bar chart:
#ggplot(data, aes(x=dependent variable, fill=independent variable)) + geom_bar(position = "dodge") + labs(title="", x="Gender", y="Frequency")
ggplot(NHANESdatabase, aes(x=BMIcat, fill=Gender)) + geom_bar(position = "dodge") + labs(title="grouped bar chart", x="BMI categories by gender", y="counts")
...
ggplot(BMI_gender, aes(x=reorder(Gender, mean), y=mean)) + geom_bar(stat = "identity", fill="pink") + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=0.2)
labs(x="Gender",
     y="",
     title = "Mean BMI",
...
ggplot(NHANESdatabase, aes(x=BMI)) + 
...
ggplot(NHANESdatabase, aes(x=BMI)) + geom_histogram(bins=30, color="lightpink", fill="pink") + labs(title="colored histogram with 20 bins", x="BMI", y="count")
```
...
ggplot(NHANESdatabase, aes(x=BMI, y=..density..)) + geom_histogram(bins=20, color="darkgreen", fill="pink") + scale_y_continuous(labels = scales::percent) + labs(title="BMI", y="percent", x="BMI")
```

d: Create a histogram of a different continuous variable: HOMEWORK
```{r}
ggplot(NHANESdatabase, aes(x=Age)) + geom_histogram(bins=30, color="darkblue", fill="lightblue") + labs(title="Age histogram with 20 bins", x="Age", y="count")
```
...
ggplot(NHANESdatabase, aes(x="", y=DR1TPROT)) + geom_boxplot() + labs(y="total protein consumed daily in grams", x="", title="Box plot")
```
...
ggplot(NHANESdatabase, aes(x=Race, y=DR1TPROT)) + geom_boxplot(fill="lightpink") + labs(x="Race", y="DR1TPROT", title="daily protein intake in grams")
